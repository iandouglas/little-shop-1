...
RSpec.describe 'As a user', type: :feature do
  before :each do
    ...
    allow_any_instance_of(ApplicationController).to receive(:current_user).and_return(@u34)
  end
  context "on an item show page" do
    it "I can see any bulk discounts for that item" do
      visit item_path(@i19)

      expect(page).to have_content("5% off when you buy 5")
      expect(page).to have_content("10% off when you buy 15")
      expect(page).to have_content("15% off when you buy 30")
    end
  end

  context "in my cart" do
    it "I can see bulk discounts applied to appropriate items" do
      visit item_path(@i19)
      ...
      expected = (@i19.current_price * 5) * (1.00 - @d1.percentage)

      expect(page).to have_content("Subtotal: $#{expected}")
